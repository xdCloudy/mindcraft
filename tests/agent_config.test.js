...
test('validateAgentSettings accepts root launcher keys but excludes them from agent config', () => {
    const config = validateAgentSettings({ profile: { name: ' Andy ' }, profiles: ['./andy.json'], mindserver_port: 8080, auto_open_ui: true, block_place_delay: 25 });
...
test('agent names reject path/control syntax before runtime use', () => {
    ...
    assert.throws(() => validateAgentSettings({ profile: { name: '../escape' } }), /Invalid agent name/);
});

test('validateAgentSettings rejects unknown per-agent settings', () => {
    assert.throws(() => validateAgentSettings({ profile: { name: 'Andy' }, typo_setting: true }), /Unknown settings/);
});
