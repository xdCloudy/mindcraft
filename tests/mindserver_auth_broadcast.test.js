...
test('public MindServer does not broadcast control data to unauthorized sockets', async () => {
    ...
    const unauthorized = connectSocket(url, {
        auth: { controlToken: 'wrong' },
        reconnection: false,
    });
    const control = connectSocket(url, {
        auth: { controlToken: 'control-secret' },
        reconnection: false,
    });
    const agent = connectSocket(url, {
        auth: { agentToken: 'agent-secret' },
        reconnection: false,
    });
