...
export class Claude {
    ...
    constructor(model_name, url, params) {
        ...
        this.requestTimeoutMs = Number.isInteger(configured) && configured > 0 ? configured : DEFAULT_REQUEST_TIMEOUT_MS;
        const config = {};
    ...
    async sendRequest(turns, systemMessage, _stopSeq = '***', { signal } = {}) {
        ...
        try {
            ...
            if (!params.max_tokens) {
                if (params.thinking?.budget_tokens) params.max_tokens = params.thinking.budget_tokens + 1000;
                else params.max_tokens = 4096;
            }
            ...
            if (textContent) res = textContent.text;
            else {
                console.warn('No text content found in the response.');
                res = 'No response from Claude.';
            }
        } catch (err) {
            if (String(err?.message).includes('does not support image input')) return 'Vision is only supported by certain models.';
